...
R.db.zips.aggregate({$group:{_id: "$city", state:{$first: "$state"}, zip_codes:{$addToSet: "$_id"}}},{$match:{zip_codes:{$size: 2}}},{$sort:{state:1 , _id:1}})


4. Display the least populated city in each state

R.  db.zips.aggregate({$group:{_id: {"city": "$city", "state": "$state"}, "city_pop": {$sum: "$pop"}}},
{$sort: {city_pop : 1}},{$group:{_id: "$_id.state", city:{$first: "$_id.city"}, popupation: {$first: "$city_pop"}}},{$sort: {"_id":1}})
